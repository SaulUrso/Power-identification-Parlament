...
def nn_train(X_train_embed, X_val_embed, y_train, y_val):
    ...
    train(DataLoader(td, batch_size=1),
            ffnn,
            nn.MSELoss(),
    ...
    precisiontfidf, recalltfidf, fscoretfidf, _ = precision_recall_fscore_support(tensor_y_val.numpy().astype(np.float32), 
                                                                                    torch.where(predtfidf>0.5,torch.tensor(1),torch.tensor(0)).numpy().astype(np.float32), 
                                                                                    average="macro")
...
def k_fold_tfidf(filename,classifier1 = LogisticRegression(class_weight="balanced", max_iter=500, penalty=None)):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(training_set)):
        ...
        precisiontfidf, recalltfidf, fscoretfidf=nn_train(X_train_tfidf.toarray(), x_val_tfidf.toarray(), y_train, y_val)
...
def k_fold_w2v(filename,classifier2 = LogisticRegression(class_weight="balanced", max_iter=500, penalty=None)):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(training_set)):
        ...
        precisionw2v, recallw2v, fscorew2v=nn_train(np.array(X_train_w2v), np.array(X_val_w2v), y_train, y_val)
...
def k_fold_fstxt(filename,classifier3 = LogisticRegression(class_weight="balanced", max_iter=500, penalty=None)):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(training_set)):
        ...
        precisiontfstxt, recallfstxt, fscorefstxt=nn_train(np.array(X_train_fstxt), np.array(X_val_fstxt), y_train, y_val)
