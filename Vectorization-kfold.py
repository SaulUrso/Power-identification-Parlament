...
def documents_vector(documents, model):
    ...
    for doc in documents:
        ...
        if len(doc_words) > 0:
            document_vectors.append(np.mean(model[doc_words], axis=0))
        else:
            document_vectors.append(np.zeros(model.vector_size))
    return document_vectors


def k_fold_tfidf(filename):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(training_set)):
        ...
        classifier1 = LogisticRegression(class_weight="balanced", max_iter=500, penalty=None)
...
def k_fold_w2v(filename):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(training_set)):
        ...
        # w2v model training
        modelw2v = gensim.models.Word2Vec(preproc_training_set, size=150, window=10, min_count=2, workers=10, iter=10)
        classifier2 = LogisticRegression(class_weight="balanced", max_iter=500, penalty=None)
...
def k_fold_fstxt(filename):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(training_set)):
        ...
        modelfstxt = gensim.models.fasttext.FastText(
            sentences=preproc_training_set, size=150, window=10, min_count=2, workers=10, iter=10)
        classifier3 = LogisticRegression(class_weight="balanced", max_iter=500, penalty=None)
        # documents vectorization
